...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="chinabond/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">估价净价</th>
									<th class="center">估价收益率</th>
									<th class="center">估价修正久期</th>
									<th class="center">估价凸性</th>
									<th class="center">可信度</th>
									<th class="center">日终估价全价</th>
									<th class="center">日终应计利息</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.VALUATION_NET_PRICE}</td>
											<td class='center'>${var.VALUATION_RETURN}</td>
											<td class='center'>${var.DURATION}</td>
											<td class='center'>${var.CONVEXITY}</td>
											<td class='center'>${var.RELIABILITY}</td>
											<td class='center'>${var.VALUATION_PRICE_END}</td>
											<td class='center'>${var.INTEREST_END}</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=path%>/chinabond/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 400;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=path%>/chinabond/goEdit.do?CHINABOND_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 400;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>chinabond/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function fromExcel(){
			 ...
			 diag.URL = '<%=path%>/chinabond/goUploadExcel.do';
			 diag.Width = 450;
			 diag.Height = 260;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>chinabond/excel.do';
		}
	</script>
