...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="bondinfo/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">停牌状态</th>
									<th class="center">停牌原因</th>
									<th class="center">发行面值</th>
									<th class="center">最新面值</th>
									<th class="center">票面利率%</th>
									<th class="center">发行价格 元</th>
									<th class="center">起息日期</th>
									<th class="center">止息日期</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.PAR_VALUE_ISSUE}</td>
											<td class='center'>${var.PAR_VALUE_LAST}</td>
											<td class='center'>${var.COUPON_RATE}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>bondinfo/goAdd.do';
			 diag.Width = 1050;
			 diag.Height = 600;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>bondinfo/delete.do?BONDINFO_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>bondinfo/goEdit.do?BONDINFO_ID='+Id;
			 diag.Width = 1050;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>bondinfo/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function fromExcel(){
			 ...
			 diag.URL = '<%=basePath%>bondinfo/goUploadExcel.do';
			 diag.Width = 450;
			 diag.Height = 260;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>bondinfo/excel.do';
		}
	</script>
