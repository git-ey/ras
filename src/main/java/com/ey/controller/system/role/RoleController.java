package com.ey.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public ModelAndView add()throws Exception{
		...
		try{
			...
			roleService.add(pd);
			logManager.save(Jurisdiction.getUsername(), "新增角色:"+pd.getString("ROLE_NAME"));
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView edit()throws Exception{
		...
		try{
			pd = this.getPageData();
			roleService.edit(pd);
			logManager.save(Jurisdiction.getUsername(), "修改角色:"+pd.getString("ROLE_NAME"));
	...
	public Object deleteRole(@RequestParam String ROLE_ID)throws Exception{
		...
		try{
			...
			if(roleList_z.size() > 0){
				...
				logManager.save(Jurisdiction.getUsername(), "删除角色ID为:"+ROLE_ID);
				errInfo = "success";
				}
			}
		} catch(Exception e){
	...
	public void saveMenuqx(@RequestParam String ROLE_ID,@RequestParam String menuIds,PrintWriter out)throws Exception{
		...
		logManager.save(Jurisdiction.getUsername(), "修改角色菜单权限，角色ID为:"+ROLE_ID);
		PageData pd = new PageData();
		try{
	...
	public void saveB4Button(@RequestParam String ROLE_ID,@RequestParam String menuIds,@RequestParam String msg,PrintWriter out)throws Exception{
		...
		logManager.save(Jurisdiction.getUsername(), "修改"+msg+"权限，角色ID为:"+ROLE_ID);
		PageData pd = new PageData();
		pd = this.getPageData();
		try{
			if(null != menuIds && !"".equals(menuIds.trim())){
